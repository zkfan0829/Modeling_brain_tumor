...
import os
os.environ.setdefault("ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS", "16")
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    method = args.method
    ROOT = Path( args.root)
    CSV_SUMMARY = Path( ROOT / "summary.csv")
    CSV_PARAMS = Path(ROOT / "rigid_params.csv")
    NUM_SAMPLES_PER_CASE = int(5)
    SEED = int(42)
    ...
    with torch.inference_mode():
        for idx in tqdm(test_indices, total=len(test_indices), desc="Evaluating", unit="case", leave=False):
            ...
            from classical_rigid import classical_rigid_predict
            # Build optional mask for focusing registration on brain only
            fixed_mask_for_reg =  None  # threshold to avoid empty/near-empty

            case_logs = (Path(args.out_csv).parent / "elx_logs" / f"case_{case_id}")
            case_logs.mkdir(parents=True, exist_ok=True)

            T_pred, pred_params = classical_rigid_predict(
                ct_t,                      # fixed (CT)
                mr_t,                      # moving (MR)
                ct_spacing_xyz,
                fixed_mask=fixed_mask_for_reg,
                method=method,
                output_directory=str(case_logs),
                log_to_console=False,      # set True if you want it on stdout
                log_to_file=True,
            ...
            # For logging (fill the columns; if pred_params is None, use NaN)
            if pred_params is None:
                pred_rx_deg = pred_ry_deg = pred_rz_deg = float('nan')
                pred_tx_mm  = pred_ty_mm  = pred_tz_mm  = float('nan')
            else:
                pred_rx_deg, pred_ry_deg, pred_rz_deg, pred_tx_mm, pred_ty_mm, pred_tz_mm = pred_params.tolist()
            ...
            row = {
                ...
                "pred_rx_deg": pred_rx_deg, "pred_ry_deg": pred_ry_deg, "pred_rz_deg": pred_rz_deg,
                "pred_tx_mm":pred_tx_mm, "pred_ty_mm": pred_ty_mm, "pred_tz_mm": pred_tz_mm,
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--threads", type=int, default=16, help="CPU threads for ITK/SimpleITK/ANTs")
    ap.add_argument("--model", type=str, default="CNN",
                    help="Pick the model [CNN/VIT]")
    ap.add_argument("--method", type=str, default="elastix",
                choices=["elastix","ants","sitk"])
...
if __name__ == "__main__":
    args = parse_args()
        ...
        # Configure threading for all backends you might use
    try:
        import itk
        itk.MultiThreaderBase.SetGlobalDefaultNumberOfThreads(args.threads)
    except Exception:
        pass
    ...
    try:
        import ants
        ants.set_number_of_threads(args.threads)
    except Exception:
        pass
